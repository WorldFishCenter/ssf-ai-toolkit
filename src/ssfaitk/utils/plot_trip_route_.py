...
def plot_trip_route(
    ...
    if effort_col and effort_col in trip_df.columns:
        # Fishing segments (red, thicker)
        fishing_df = trip_df[trip_df[effort_col] == 1]
        if len(fishing_df) > 0:
            fishing_coords = fishing_df[[lat_col, lon_col]].values.tolist()
            folium.PolyLine(
                fishing_coords,
                color='#d62728',  # Red
                weight=4,
        ...
        # Non-fishing segments (blue, medium)
        non_fishing_df = trip_df[trip_df[effort_col] == 0]
        if len(non_fishing_df) > 0:
            non_fishing_coords = non_fishing_df[[lat_col, lon_col]].values.tolist()
            folium.PolyLine(
                non_fishing_coords,
                color='#1f77b4',  # Blue
                weight=3,
