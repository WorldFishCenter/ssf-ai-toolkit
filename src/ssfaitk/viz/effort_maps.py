...
def plot_effort_tracks(
    ...
    Args:
        ...
        effort_col: Effort column name (auto-detected if None, 0=non-fishing, 1=fishing)
        ...
        alpha: Line transparency (default: 0.6)
        linewidth: Width of track lines (default: 1.5)
    ...
    Examples:
        ...
        >>> plot_effort_tracks(df, 'tracks.png')
        ...
        >>> # All work:
        >>> plot_effort_tracks(df1, 'tracks1.png')
        >>> plot_effort_tracks(df2, 'tracks2.png')
        >>> plot_effort_tracks(df3, 'tracks3.png')
    """
...
def plot_effort_heatmap(
    ...
    Args:
        df: DataFrame with track points and effort predictions
        output_path: Where to save the plot (optional)
        effort_col: Effort column name (auto-detected if None, 0/1)
        lat_col: Latitude column name (auto-detected if None)
        lon_col: Longitude column name (auto-detected if None)
    ...
    Examples:
        # Auto-detect all columns
        >>> plot_effort_heatmap(df, 'heatmap.png')
        
        # Specify custom columns and grid size
        >>> plot_effort_heatmap(
        ...     df,
        ...     'heatmap.png',
        ...     effort_col='fishing_activity',
        ...
        >>> plot_effort_heatmap(df, 'heatmap_mean.png', aggregation='mean')
        >>> plot_effort_heatmap(df, 'heatmap_count.png', aggregation='count')
    """
    # Resolve column names (auto-detect or use provided)
    if COLUMN_MAPPER_AVAILABLE:
        effort_col = resolve_column_name(df, 'effort', effort_col, required=True)
        lat_col = resolve_column_name(df, 'latitude', lat_col, required=True)
        lon_col = resolve_column_name(df, 'longitude', lon_col, required=True)
        logger.debug(f"Resolved columns: effort={effort_col}, lat={lat_col}, lon={lon_col}")
    else:
        # Fallback
        effort_col = effort_col or 'is_fishing'
        lat_col = lat_col or 'latitude'
    ...
    stats_text = (
        ...
        f"Grid: {grid_shape[0]}×{grid_shape[1]} cells\n"
        f"Max Cell Value: {max_cell:.1f}\n"
        f"Hotspots (>90th %ile): {n_hotspots}"
    )
...
def plot_effort_hotspots(
    ...
    """
    Identify and visualize high-effort fishing hotspots.
    ...
    Examples:
        # Auto-detect all columns
        >>> plot_effort_hotspots(df, 'hotspots.png')
        
        # Specify custom threshold
        >>> plot_effort_hotspots(
        ...     df,
        ...     'hotspots.png',
        ...     percentile_threshold=75  # Top 25% instead of top 10%
        ... )
        
        # Custom columns and finer grid
        >>> plot_effort_hotspots(
        ...     df,
        ...     'hotspots.png',
        ...     effort_col='fishing_activity',
        ...     lat_col='lat',
        ...     grid_size=0.005  # Finer resolution
        ... )
    """
    # Resolve column names (auto-detect or use provided)
    if COLUMN_MAPPER_AVAILABLE:
...
def plot_effort_comparison(
    ...
    Args:
        ...
        lon_col: Longitude column name (auto-detected if None)
        trip_col: Trip identifier column name (auto-detected if None)
        grid_size: Grid cell size for heatmap (default: 0.01)
        figsize: Figure size (default: (18, 10))
    ...
    Examples:
        # Auto-detect all columns
        >>> plot_effort_comparison(df, 'comparison.png')
...
def generate_effort_report(
    ...
    Args:
        ...
        grid_size: Grid size for spatial aggregation (default: 0.01)
        prefix: Filename prefix for outputs (default: 'effort_analysis')
    ...
    Examples:
        # Auto-detect all columns
        >>> outputs = generate_effort_report(df, 'outputs/')
        >>> print(outputs['heatmap'])  # Path to heatmap
        ...
        # Custom grid size and prefix
        >>> outputs = generate_effort_report(
        ...     df,
        ...     'outputs/',
        ...     grid_size=0.02,
        ...     prefix='zanzibar_fishing'
        ... )
    """
    # Resolve column names once for all plots
    if COLUMN_MAPPER_AVAILABLE:
        effort_col = resolve_column_name(df, 'effort', effort_col, required=True)
        lat_col = resolve_column_name(df, 'latitude', lat_col, required=True)
        lon_col = resolve_column_name(df, 'longitude', lon_col, required=True)
        trip_col = resolve_column_name(df, 'trip_id', trip_col, required=True)
        logger.info(f"Resolved columns: effort={effort_col}, lat={lat_col}, "
                   f"lon={lon_col}, trip={trip_col}")
    else:
        # Fallback
        effort_col = effort_col or 'is_fishing'
    ...
    fig1 = plot_effort_tracks(
        df, None, effort_col, lat_col, lon_col, trip_col
    )
    ...
    fig2 = plot_effort_heatmap(
        df, None, effort_col, lat_col, lon_col, grid_size
    )
    ...
    fig4 = plot_effort_comparison(
        df, None, effort_col, lat_col, lon_col, trip_col, grid_size
    )
